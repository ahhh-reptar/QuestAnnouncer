...
function QuestAnnouncerFu:OnTooltipUpdate()
	...
	if QuestAnnouncer:GetProgressType() == "all" then
		cat:AddLine("text", L["OPT_PROGRESS_ALL"])
	elseif QuestAnnouncer:GetDisplayType() == "last" then
		cat:AddLine("text", L["OPT_PROGRESS_LAST"])
	end
...
function QuestAnnouncerFu:GetProgressType()
	return QuestAnnouncer:GetProgressType()
end

function QuestAnnouncerFu:SetProgressType(name)
	QuestAnnouncer:SetProgressType(name)
end